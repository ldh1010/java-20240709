...
public class DataType {
	...
	public static void main(String[] args) {
		...
		// byte : 1byte(8bit)의 크기를 가지는 정수형 데이트 타입
		// -128 ~ +127
		byte byte1 = -128;
		// byte1 = 128;
		...
		// 형변환 : 데이터 타입 서로 다른 변수를 옯겨 닮는 것
		
		// 자동 형변환 : 작은 데이터 타입의 변수를 큰 데이터 타입의 변수에 옮겨 닮을 때 발생
		// 강제 형변환 : 큰 데이터 타입의 변수를 작은 데이터 타입의 변수에 옯겨 닮을 때 발생
		...
		// 문자열(String)의 주요 기능
		String statenment = "  My name is loki  ";
		
		// 1. 문자열 결합
		// + 혹은 concat(문자열)으로 두 문자열을 연결할 수 있음
		System.out.println(statenment + "@@@");
		System.out.println(statenment.concat("!!!"));
		
		// 2. 문자열 비교
		// equals(문자열) : 두 문자열이 동등한지 비교
		// compareTo(문자열) : 두 문자열이 사전순으로 비교(대소문자 구분)
		// compareToIfnoreCase(문자열) : 두 문자열을 사전순으로 비교(대소문자 구분X)
		System.out.println(statenment.equals("  My name is loki  "));
		System.out.println(statenment.compareTo("  My name is loki  "));
		
		// 3. 문자열 길이
		// length() : 문자열의 길이를 반환
		System.out.println(statenment.length());
		
		// 4. 문자열 변환
		// toUpperCase() : 모든 문자를 대문자로 변경
		// toLowerCase() : 모든 문자를 소문자로 변경
		// trim() : 문자열의 앞뒤 공백을 제거
		System.out.println(statenment.toUpperCase());
		System.out.println(statenment.toLowerCase());
		System.out.println(statenment.trim());
		
		// 5. 문자열 찾기
		// indexOf(문자열) : 찾고자하는 문자열의 처음 찾은 위치의 인덱스 번호를 반환
		// lastIndexOf(문자열) : 찾고자하는 문자열의 마지막 찾은 위치의 인덱스 번호를 반환
		statenment = "내가 그린 기린 그림은 잘 그린 기린 그림이고 네가 그린 기린 그림은 잘 못그린 기린 그림이다.";
		System.out.println(statenment.indexOf("기린"));
		System.out.println(statenment.indexOf("사자"));
		System.out.println(statenment.lastIndexOf("기린"));
		System.out.println(statenment.lastIndexOf("사자"));
		
		// 6. 부분 문자열
		// subsring(시작인덱스), substring(시작인덱스, 종료인덱스) : 문자열을 원하는 크기만큼 추출
		System.out.println(statenment.substring(26));
		System.out.println(statenment.substring(26, 31));
